...
export async function regulation_watcher_brief(cfg) {
  ...
  const body = await ask({ system: systemPrompt(cfg, "regulation_watcher"), tools: ["web_search"], model: cfg.staff.regulation_watcher.model,
...
export async function trend_researcher_report(cfg) {
  ...
  const body = await ask({ system: systemPrompt(cfg, "trend_researcher"), tools: ["web_search"], model: cfg.staff.trend_researcher.model,
...
export async function editor_meeting(cfg) {
  ...
  const opinions = await Promise.all(roles.map(async ([id, task]) => [id, await ask({ system: systemPrompt(cfg, id), model: cfg.staff[id].model, user: `기획 회의 참석 의견을 내세요.\n${context}\n\n요청: ${task}` })]));
  const opText = opinions.map(([id, t]) => `## ${id} 의견\n${t}`).join("\n\n");
  const minutes = await ask({ system: systemPrompt(cfg, "editor"), model: cfg.staff.editor.model,
...
export async function editor_plan(cfg) {
  ...
  const lastPlan = latest(items, i => i.type === "기획안");
  const body = await ask({ system: systemPrompt(cfg, "editor"), model: cfg.staff.editor.model,
...
export async function blog_writer_write(cfg, maxPosts = 3) {
  ...
  for (const c of todo) {
    const body = await ask({ system: systemPrompt(cfg, "blog_writer"), model: cfg.staff.blog_writer.model, max_tokens: 7000,
...
export async function regulation_reviewer_review(cfg) {
  ...
  for (const it of q) {
    ...
    const r = await askJSON({ system: systemPrompt(cfg, "regulation_reviewer"), model: cfg.staff.regulation_reviewer.model, dry: { verdict: "통과", items: [], reason: "DRY", flag: false },
...
export async function upload_recorder_instruct(cfg) {
  const { items, w } = await ctx();
  const approved = items.filter(i => i.status === "승인" && i.line === "블로그");
  ...
  if (todo.length) {
    const body = await ask({ system: systemPrompt(cfg, "upload_recorder"), model: cfg.staff.upload_recorder.model,
      user: `아래 승인 글의 「발행 지시서 ${w}」를 마크다운 표로: 순서 / 글 제목 / 지점 계정 / 권장 발행 시각(독자 시간대 근거) / 촬영 필요 컷 요약 / 주의. 마지막에 "발행 후 할 일" 3줄.\n${todo.map(t => `- ${t.title} (${t.stores.join(",")}) ${t.url}`).join("\n")}` });
...
export async function editor_weekly_memo(cfg) {
  const { items, w } = await ctx(); const wk = byWeek(items, w);
  const memo = await ask({ system: systemPrompt(cfg, "editor"), model: cfg.staff.editor.model,
