...
const IDS = { "주간 마케팅 편집장": "editor", "쇼츠 트렌드 리서처": "trend", "규제 감시자": "reg", "블로그 작가": "writer", "규제 검수관": "reviewer", "업로드 기록원": "uploader", "POP 디자이너": "pop", "소설 읽는 사람": "novel", "영화 보는 사람": "film", "시 읽는 사람": "poem" };
...
export async function snapshot(cfg) {
  ...
  const snap = {
    ...
    pops,
    meeting,
    schedule: Object.values(cfg.jobs).filter(j => !j.kst.includes("분마다")).map(j => ({ when: j.kst, who: j.run.map(r => ({ regulation_watcher: "reg", trend_researcher: "trend", editor: "editor", blog_writer: "writer", regulation_reviewer: "reviewer", upload_recorder: "uploader", pop_designer: "pop", panel_poem: "poem" })[r.split(":")[0]]).filter(Boolean), what: j.run.map(r => r.split(":")[1]).join(" · ") })),
